    ...
    if fSETOR == 'TECNOLOGIA DA INFORMAÇÃO':
        if senha == '69':
                            ...
                            if niveldaocorrencia != "Selecione":
                                if solicitante != "Selecione":
                                    if setor != "Selecione":   
                                            ...
                                            if att:
                                               st.balloons()
                                               cursor.execute("INSERT INTO ABERTURA (OS,SOLCITANTE,SETOR,TIPO_DE_OCORRENCIA,NIVEL_DA_OCORRENCIA,DATA) VALUES (?, ?, ?, ?, ?,?)", (cnt3 , str(solicitante), str(setor), str(status),str(niveldaocorrencia),data))
                                               conn.commit()
                                               conn.close()
            ...
            with tab3:
                ...
                with statuses:   
                   Nmr = st.number_input("Selecione o numero da OS",min_value=1,value=1,placeholder="Selecione")
